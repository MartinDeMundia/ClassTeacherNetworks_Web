...
$adm_search="";
if($ss=="false"){
	$adm_search="and sudtls.adm='$adms'  ";
}else{
	$adm_search="";
}
//echo $adm_search;
...
If ($Stream =="ALL") {
                If ($exm =="all") {
                    $q1 = "SELECT `".$f2."term`.`adm`, `sudtls`.`Name` ,`".$f2."term`.`Stream`, `".$f2."term`.`PosStream` , `".$f2."term`.`PosClass`,subno AS 'No. of Sub',CAT1, CAT2, CAT3, ENDTERM,`".$f2."term`.`TotalMarks`, `".$f2."term`.`TotalPercent` , `".$f2."term`.`POINTS` ,`".$f2."term`.`Grade`, (`sudtls`.`kcpe`- `".$f2."term`.POINTS) as `VALUE_ADDITION`  FROM `school_5`.`".$f2."term` INNER JOIN `school_5`.`sudtls` ON (`".$f2."term`.`Adm` = `sudtls`.`Adm`) WHERE Year='$year' $adm_search and Term='$term'   ";
                    ...
                    If (2==8) {
                        ...
                        $q1 = "SELECT `".$f2."$exm`.`adm`, `sudtls`.`Name`, `".$f2."$exm`.`Form` ,`".$f2."$exm`.`Stream`, `".$f2."$exm`.`PosStream` , `".$f2."$exm`.`PosClass`,subno,`".$f2."$exm`.`TotalMarks`, `".$f2."$exm`.`TotalScore` , `".$f2."$exm`.`TotalPercent`  , `".$f2."$exm`.`Grade`, `".$f2."$exm`.`POINTS` ,(`sudtls`.`kcpe`- `".$f2."$exm`.POINTS) as `VALUE_ADDITION`  FROM `school_5`.`".$f2."$exm` INNER JOIN `school_5`.`sudtls` ON (`".$f2."$exm`.`Adm` = `sudtls`.`Adm`) where Year='$year' $adm_search  and Term='$term'  and ".$f2."$exm.form='$form' ;";
			}
			}
                ...
                If ($exm =="all") {
                    ...
                    If (2==8) {
                        ...
                        $q1 = "SELECT `".$f2."$exm`.`adm`, `sudtls`.`Name`, `".$f2."$exm`.`Form` ,`".$f2."$exm`.`Stream`, `".$f2."$exm`.`PosStream` , `".$f2."$exm`.`PosClass`,subno,`".$f2."$exm`.`TotalMarks`, `".$f2."$exm`.`TotalScore` , `".$f2."$exm`.`TotalPercent`  , `".$f2."$exm`.`Grade`, `".$f2."$exm`.`POINTS` ,`sudtls`.`kcpe`- `".$f2."$exm`.POINTS as `VALUE_ADDITION`  FROM `school_5`.`".$f2."$exm` INNER JOIN `school_5`.`sudtls` ON (`".$f2."$exm`.`Adm` = `sudtls`.`Adm`) where Year='$year' $adm_search  and Term='$term' and ".$f2."$exm.stream='$Stream' and ".$f2."$exm.form='$form' ;";
			}
			}
...
$pdf->AddPage();
 ...
 while($rs=mysqli_fetch_assoc($qq)){
	...
	//$pdf->Image('uploads/logo.png',250,6,30);
	...
	$pdf->Image('uploads/student_image/'.$rs['adm'].'.jpg',170,6,20);
	$adm=$rs['adm'];
	}
	else{
		$pdf->Image('uploads/user.png',170,6,20);
...
$qsc11="SELECT concat((TotalScore),' ' ,gradingscale.grade) as TotalScore,PosClass  FROM cat1 left join gradingscale ON (round(TotalPercent)>=MIN AND round(TotalPercent)<=MAX)  WHERE adm='".$rs['adm']."' AND cat1.term='$term' AND year='$year' and Subject='".$sr['Abbreviation']."'";
			$score="";
			$pos="";
	...
	$pdf->Cell(20	,5,$score.'('.$pos.')',1,0,'C');
		...
		$qsc12="SELECT concat((TotalScore),' ' ,gradingscale.grade) as TotalScore,PosClass  FROM cat2 left join gradingscale ON (round(TotalPercent)>=MIN AND round(TotalPercent)<=MAX)  WHERE adm='".$rs['adm']."' AND cat2.term='$term' AND year='$year' and Subject='".$sr['Abbreviation']."'";
			$score="";
			$pos="";
	...
	$pdf->Cell(20	,5,$score.'('.$pos.')',1,0,'C');
		...
		$qsc13="SELECT concat((TotalScore),' ' ,gradingscale.grade) as TotalScore,PosClass  FROM cat3 left join gradingscale ON (round(TotalPercent)>=MIN AND round(TotalPercent)<=MAX)  WHERE adm='".$rs['adm']."' AND cat3.term='$term' AND year='$year' and Subject='".$sr['Abbreviation']."'";
			$score="";
			$pos="";
	...
	$qsc1="SELECT concat(round(TotalScore),' ' ,gradingscale.grade) as TotalScore,PosClass  FROM endterm left join gradingscale ON (round(TotalPercent)>=MIN AND round(TotalPercent)<=MAX)  WHERE adm='".$rs['adm']."' AND endterm.term='$term' AND year='$year' and Subject='".$sr['Abbreviation']."'";
			$score="";
			$pos="";
	...
	//TOTAL
		$qsc1v="SELECT concat(round(TotalScore),' ' ,rgrade(round(TotalScore))) as TotalScore,round(TotalScore) as c FROM ve   WHERE adm='".$rs['adm']."' AND ve.term='$term' AND year='$year' and Subject='".$sr['Abbreviation']."'";
			$score="";
			$score1="";
			$pos="";
			$remarks="";
 ...
 while($rs34=mysqli_fetch_assoc($qq34)){
	...
	$pdf->SetFont('Arial','U',10);
	$pdf->Write(5,'      '.round($rs['TotalMarks'],2).'       ');
	$pdf->SetFont('Arial','B',10);
	$pdf->Write(5,'M/S.');
	$pdf->SetFont('Arial','U',10);
	$pdf->Write(5,'    '.round($rs['TotalPercent'],2).'    ');
	$pdf->SetFont('Arial','B',10);
	$pdf->Write(5,'M/G');
	$pdf->SetFont('Arial','U',10);
...
if($data2==0){
	...
	//Bar diagram
	$data = array($term => round($rs['TotalPercent'],2), $t1 => round($data1,2), $t2 => round($data2,2));
	$pdf->SetFont('Arial', '', 10);
$valX = $pdf->GetX();
$valY = $pdf->GetY();
...
$pdf->Output('f','PDFS/report_forms.pdf');
?>
