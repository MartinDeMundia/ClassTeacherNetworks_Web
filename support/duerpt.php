...
while($row2s=mysqli_fetch_assoc($rs)){
	...
	// Title
	$this->Cell(0,10,strtoupper($_SESSION['subject']."  ".$_SESSION['category']."  ".$_SESSION['class'].'  ITEMS OVERDUE' ) ,0,0,'C');
...
$pdf->AddPage();
$pdf->SetFont('Arial','',8);
$q="SELECT  borrowed.date,date_format(duedate,'%d %M %Y') as duedate,books.status,user_code,serial,isbn,title,form,category,subject FROM books LEFT JOIN borrowed ON (borrowed.book_id=books.id) WHERE  DATEDIFF( duedate,NOW())<0 $serial  $isbn $title $author $category $subject ";
		//echo $q;
					$result=$con->query($q);
