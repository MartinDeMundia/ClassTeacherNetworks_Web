        ...
        If (($exm) == "endterm") {
            ...
            while($row = $result->fetch_assoc()) {
                ...
                $p = 0;
                    ...
                    $sql2 = "SELECT  adm,CONCAT((TotalScore)) AS TotalScore  ,gradingscale.grade FROM ".$exm." LEFT JOIN gradingscale ON (TotalPercent>=MIN AND TotalPercent<=MAX)   WHERE ".$exm.".subject='$ss' and ".$exm.".Year='$year' and ".$exm.".Form='$form' ORDER BY CODE asc";
...
$_SESSION['out']=$count;
            ...
            $strQry1 = "SELECT DISTINCT (Stream) as Stream from  $exm Where Year='$year' ";
 ...
 $_SESSION['progress_text']='Ranking  ['. strtoupper($results1['Stream']).']  '.strtoupper($results2['SUBJECT']);
					...
					$tie3=0;
                    $query = "Select Adm,subject,Code,TotalScore as Score,(TotalPercent) as ' Score(%)',grade,posclass,posstream  from  $exm where Year='$year' and Term='$term'and form='$form'  and subject='$subject' order by TotalPercent desc";
...
*/
		...
		while($results1a=mysqli_fetch_assoc($r1a)){
			...
			while($results2a=mysqli_fetch_assoc($r2a)){
				$_SESSION['progress_text']='Ranking  ['. strtoupper($results1a['Stream']).']  '.strtoupper($results2a['SUBJECT']);
				$_SESSION['out']=$count;
                    $pos = 0;
                    $pos2 = 0;
                    $subject = $results2a['SUBJECT'];
					$tie2 =0;
                    $querya = "Select Adm,subject,Code,(TotalScore) as Score,(TotalPercent) as ' Score(%)',grade,posclass,posstream  from  $exm where Year='$year' and Term='$term' and stream='$stream' and form='$form'  and subject='$subject' order by TotalPercent desc";
...
global $exm;  
		...
		while($results1cc=mysqli_fetch_assoc($r1cc)){
            ...
            if (mysqli_query($con,$s))
                                 {
									 $_SESSION['progress_text']='Processing  '. strtoupper($en);
									 $_SESSION['progress']=0;
									  $_SESSION['out']=0;
...
global $exm;
		...
		while($results3c=mysqli_fetch_assoc($r3c)){
            ...
            $s2  = "INSERT INTO $f2$exm (Year,subno,TotalMarks,Adm,Term,Stream,TotalScore,TotalPercent,Form)  SELECT Year,subno,(SUM(TotalPercent)),adm,Term,Stream,(AVG(TotalScore)),(AVG(TotalPercent)) ,Form from $exm WHERE term='$term' and Year='$year' and form='$form' GROUP BY adm ";
...
global $exm;
		...
		while($results3cc=mysqli_fetch_assoc($r3cc)){
			$_SESSION['progress_text']='Ranking  ['. strtoupper($results3cc['Adm']);
				$_SESSION['out']=$count;
                $pvv += 1;
                $adm = $results3cc['Adm'];
...
$tie =0;
                $query12 = "Select Adm,TotalMarks,TotalScore as Score,TotalPercent   from $f2$exm  where  Year='$year' and  subno>='$subjectno' and stream='$subject' and Term='$term'and form='$form'  order by TotalPercent desc";
					...
					while($results13a=mysqli_fetch_assoc($r12a)){
                        ...
                        $pos2 += 1;
                       $_SESSION['progress_text']='Ranking  ['. strtoupper($results13a['Adm']);
				$_SESSION['out']=$count;
$_SESSION['progress']=$pv;
...
$_SESSION['progress_text']='Ranking  ['. strtoupper($results13af['Adm']);
				$_SESSION['out']=$count;
$_SESSION['progress']=$pv;
...
$_SESSION['progress_text']='Ranking  ['. strtoupper($results13at['Adm']);
				$_SESSION['out']=$count;
$_SESSION['progress']=$pv;
...
;
       ...
       $_SESSION['progress_text']='term processing';
				$_SESSION['out']=0;
$_SESSION['progress']=0;
...
<div class="card"  >
						...
						<strong class="card-title "><center><?php echo strtoupper($en." - "); ?>EXAM RESULTS FOR <?php echo strtoupper($form." ". $Stream. " ($term - ". $year. ")"); ?></center></strong>
		<div class="card-body ">
		<div class="table-responsive">
                            <table class="table table-bordered dataTables-example" id="table_export"  style="font-size:12px; color:;">
                              <thead>
                                <tr>
                                  <th scope="col">RNK</th>
								  <th scope="col">ADM</th> 
				...
				$pv+=1;
						  ...
						  ?>
						    <tr>
                              ...
                              <td><?php echo round($rs['TotalMarks']); ?></td>
							  <td><?php echo round($rs['TotalScore']); ?></td>
...
<div class="row" id="mr">
	 <div class="col-lg-12">
                <div class="ibox">
                    <div class="ibox-title">
                        <h5><strong class="card-title "><center><?php echo strtoupper($enn." - "); ?>EXAM RESULTS FOR <?php echo strtoupper($form." ". $Stream. " ($term - ". $year. ")"); ?></center></strong></h5>
                        <div class="ibox-tools">
                            <a class="collapse-link">
                                <i class="fa fa-chevron-up"></i>
                            </a>
        ...
        $(document).ready(function(){
            $('.dataTables-example').DataTable({
                ...
                buttons: [
                    { extend: 'copy'},
                    {extend: 'csv'},
                    {extend: 'excel', title: '<?php echo strtoupper($en." - "); ?>EXAM RESULTS FOR <?php echo strtoupper($form." ". $Stream. " ($term - ". $year. ")"); ?>'},
                    {extend: 'pdf', title: '<?php echo strtoupper($en." - "); ?>EXAM RESULTS FOR <?php echo strtoupper($form." ". $Stream. " ($term - ". $year. ")"); ?>',pageSize: 'A4',orientation: 'PORTRAIT'},
