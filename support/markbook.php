...
*/
		...
		while($results1a=mysqli_fetch_assoc($r1a)){
			...
			while($results2a=mysqli_fetch_assoc($r2a)){
					...
					$tie2 =0;
                    $querya = "Select Adm,subject,Code,TotalScore as Score,ROUND(TotalPercent,1) as ' Score(%)',grade,posclass,posstream  from  $exm where Year='$year' and Term='$term' and stream='$stream' and form='$form'  and subject='$subject' order by TotalPercent desc";
...
<div class="card"  >
						...
						<strong class="card-title "><center>MARKBOOK  <?php echo strtoupper($form." ". $Stream. " ($term - ". $year. ")"); ?></center></strong>
		...
		<span width="100%" height="100%"></span>
		 <img class="" src="uploads/preloader.gif" id="loader">
                            <table class="table table-bordered davtaTables-example" style="font-size:12px; color:;" id="mr">
                              <thead>
                                <tr>
								  <th scope="col">ADM</th> 
...
$_SESSION['progress']=$pv;	
		...
		?>
			<td scope="col"><?php echo $score; ?><sub><?php if($pos !=="") echo " (".$pos.")" ;?></sub></td><?php
						  }
						  ?>
                          </tr>
		...
		while($qqr=mysqli_fetch_assoc($qr)){
			$processed=$qqr['c'];
              ...
              If ($Stream =="all") {
                        ...
                        $q1 = "SELECT `$exm`.`adm`, `sudtls`.`Name`, `$exm`.`Form` ,`$exm`.`Stream`, `$exm`.`PosStream` , `$exm`.`PosClass`,subno, `$exm`.`TotalScore` , `$exm`.`TotalPercent`  , `$exm`.`Grade`  FROM `school_5`.`$exm` INNER JOIN `school_5`.`sudtls` ON (`$exm`.`Adm` = `sudtls`.`Adm`) where Year='$year' and Term='$term'  and $exm.form='$form' and subno>='$subjectno' and subject='$subject'";
                        ...
                        $q1 = "SELECT `$exm`.`adm`, `sudtls`.`Name`, `$exm`.`Form` ,`$exm`.`Stream`, `$exm`.`PosStream` , `$exm`.`PosClass`,subno,`$exm`.`TotalScore`  , `$exm`.`Grade`  FROM `school_5`.`$exm` INNER JOIN `school_5`.`sudtls` ON (`$exm`.`Adm` = `sudtls`.`Adm`) where Year='$year' and Term='$term' and $exm.stream='$Stream' and $exm.form='$form' and subno>='$subjectno' and subject='$subject'";
...
<div class="card"  >
						...
						<strong class="card-title "><center>MARKBOOK  <?php echo strtoupper($form." ". $Stream. " ($term - ". $year. ")"); ?></center></strong>
		<div class="ibox-tools">
                            <a href="../PDFS/markbook2.pdf" id="p rt" download="MARKBOOK -  <?php echo strtoupper($form." ". $Stream. " ".$_SESSION['exxm']." ($term - ". $year. ")"); ?>">
                                <i class="fa fa-2x fa-print" ></i>
                            </a>
...
<?php if (cat1=="True"){?>
								    <td><?php
$cat1=0;
$qscw1m="SELECT CONCAT(TotalScore,' ',gradingscale.grade) as TotalScore,PosStream FROM cat1 left join gradingscale ON (TotalPercent>=MIN AND TotalPercent<=MAX) where adm='".$rs['adm']."' and Year='$year' and Term='$term' and subject='$subject'";
								  $subsc1m=mysqli_query($con,$qscw1m);
		while($srcvm=mysqli_fetch_assoc($subsc1m)){
			
									
									$cat1=round($srcvm['TotalScore'],2); 
		...
		$total+=$cat1;
				echo $cat1;					
								 ...
								 <?php if (cat2=="True"){
								   ...
								   <td><?php $qscw1m="SELECT CONCAT(TotalScore,' ',gradingscale.grade) as TotalScore,PosStream FROM cat2 left join gradingscale ON (TotalPercent>=MIN AND TotalPercent<=MAX)  where adm='".$rs['adm']."' and Year='$year' and cat2.Term='$term' and subject='$subject'";
								   
								  $subsc1m=mysqli_query($con,$qscw1m);
		while($srcvm=mysqli_fetch_assoc($subsc1m)){
			
				$cat2=	round($srcvm['TotalScore'],2);											
		...
		?></td>   <?php }?>
								   <?php if (cat3=="True"){
									   ...
									   $cat3=0;
									   ?>
								   <td><?php $qscw1m="SELECT CONCAT(TotalScore,' ',gradingscale.grade) as TotalScore,PosStream FROM cat3 left join gradingscale ON (TotalPercent>=MIN AND TotalPercent<=MAX) where adm='".$rs['adm']."' and Year='$year' and cat3.Term='$term' and subject='$subject'";
								  $subsc1m=mysqli_query($con,$qscw1m);
		while($srcvm=mysqli_fetch_assoc($subsc1m)){
			

			$cat3=	round($srcvm['TotalScore'],2);					
		...
		?>							  
							  ...
							  <td><?php $qscw1m="SELECT CONCAT(TotalScore,' ',gradingscale.grade) as TotalScore,PosStream FROM endterm left join gradingscale ON (TotalPercent>=MIN AND TotalPercent<=MAX) where adm='".$rs['adm']."' and Year='$year' and Term='$term' and subject='$subject'";
								  $subsc1m=mysqli_query($con,$qscw1m);
		while($srcvm=mysqli_fetch_assoc($subsc1m)){
			

									$endterm= round($srcvm['TotalScore'],2); 
        ...
        $(document).ready(function(){
            ...
            $('.dataTables-example').DataTable({
                ...
                buttons: [
                    { extend: 'copy'},
                    {extend: 'csv'},
                    {extend: 'excel', title: 'MARKBOOK  <?php echo strtoupper($form." ". $Stream. " ($term - ". $year. ")"); ?>'},
                    {extend: 'pdf', title: 'MARKBOOK  <?php echo strtoupper($form." ". $Stream. " ($term - ". $year. ")"); ?>',pageSize: 'A4',orientation: 'portrait'},
