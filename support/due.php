                ...
                <div class="ibox " id="ibox2">
                    <div class="ibox-title">
                        ...
                        <div class="ibox-tools">
                            ...
                            <a id="rpt" href="../PDFS/overdue.pdf" download="<?php echo strtoupper($_SESSION['subject']."-".$_SESSION['category']."-".$_SESSION['class'].'-ITEMS OVERDUE' ) ?>">
...
<div class="sk-spinner sk-spinner-wave">
                    ...
                    <tbody>
					<?php 
					$q="SELECT books.status,user_code,serial,isbn,title,form,category,subject FROM books LEFT JOIN borrowed ON (borrowed.book_id=books.id) WHERE  DATEDIFF( duedate,NOW())<0  $serial  $isbn $title $author $category $subject";
					//echo $q;
					$result=$con->query($q);
						   ...
						   <td><?php echo $row['form']; ?></td>
						    <td><?php echo $row['category']; ?></td>
							 ...
							 <td><?php echo $row['user_code']; ?></td>
							  <td><?php if($row['status']=="AVAILABLE"){ ?><button id="<?php echo $row['id']; ?>" class="btn btn-info btn-sm issue">Issue</button> <?php } else if($row['status']=="Lost") { ?> <small class="label label-danger">Lost</small><?php } else { ?> <small class="label label-danger">Overdue</small><?php } ?> </td>
                    </tr>
					<?php 
					}
					?>
