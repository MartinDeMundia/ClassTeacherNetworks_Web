...
while($row2s=mysqli_fetch_assoc($rs)){
	...
	// Title
	$this->Cell(0,10,strtoupper($_SESSION['subject']."  ".$_SESSION['category']."  ".$_SESSION['class'].'  '.$_SESSION['param']. ' items' ) ,0,0,'C');
...
$pdf->AddPage();
$pdf->SetFont('Arial','',8);
$q="SELECT borrowed.date,date_format(duedate,'%d %M %Y') as duedate,user_code,serial,isbn,title,form,category,subject from books LEFT JOIN borrowed ON (borrowed.book_id=books.id) where 1=1  $serial  $isbn $title $author $category $subject $param";
		//echo $q;
					$result=$con->query($q);
...
$pdf->SetTitle(strtoupper(strtoupper($_SESSION['subject']."  ".$_SESSION['category']."  ".$_SESSION['class'].'  '.$_SESSION['param']. ' items' )));
$pdf->Output('F','PDFS/items.pdf');
