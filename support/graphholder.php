...
$exm='ve';
$fm="";
$q13 = "SELECT AVG(TotalScore) as m FROM $exm WHERE Year='$year' $str and  term='".$term."' AND form='".$form."' AND SUBJECT='".$rs22['sub1']."' ";
}else{	
...
$exm=$_SESSION['exm'];
	 ...
	 $q1g1 = "SELECT COUNT(Grade) as grade2 FROM ".$fm."$exm WHERE Year='$year' $str and  term='".$term."' AND form='".$form."' AND SUBJECT='".$rs22['sub1']."' AND grade='".$rgs['Grade']."'";
	 //echo $q1;
...
';
 ...
 while($rs22=mysqli_fetch_assoc($qq22)){
	 if(strtolower($en=="all")){
	$enb='form'.$form.'term'; 
$q13 = "SELECT AVG(TotalScore) as m FROM ve WHERE Year='$year' and term='".$term."' AND form='".$form."' AND SUBJECT='".$rs22['sub1']."' ";
}else{	
...
$countnum="";
 ...
 while($rgs=mysqli_fetch_assoc($qqg)){
	...
	if(strtolower($en=="all")){
	$enb='form'.$form.'term'; 
 $count = "SELECT COUNT(Grade) as grade2 FROM ve WHERE Year='$year'  and  term='".$term."' AND grade='".$rgs['Grade']."' $str";
}else{	
...
$countnum.='{ label: "'.$rgs['Grade'].'", y: '.$resc['grade2'].' },';
			...
			if(strtolower($en=="all")){
	$enb='form'.$form.'term'; 
 $count = "SELECT COUNT(Grade) as grade2 FROM ve WHERE Year='$year'  and  term='".$term."' AND grade='".$rgs['Grade']."' ";
}else{	
 $count = "SELECT COUNT(Grade) as grade2 FROM ".$f2."$exm WHERE Year='$year'  and  term='".$term."' AND grade='".$rgs['Grade']."' ";
$exm=$_SESSION['exm'];
}
 $countr=mysqli_query($con,$count);
 while($resc=mysqli_fetch_assoc($countr)){
...
var chart = new CanvasJS.Chart("chartContainer", {
	...
	theme: "light2",
	title: {
		text: " GRADE ANALYSIS (<?php echo strtoupper($form.'  '.$en.'    -   '.$term.'  '.$year); ?>)"
	},
      axisX:{
...
var chart = new CanvasJS.Chart("chartContainer4", {
	...
	} <?php if(strtolower($Stream)=="all") { ?>,{
		type: "area",
		name: "STREAM MEAN SCORE",
		showInLegend: true,
		dataPoints: [
			<?php echo $data1; ?>
	{ label: "", y: 0 }]
...
var chart = new CanvasJS.Chart("chartContainer3", {
	...
	theme: "light2",
	title: {
		text: " SUBJECTS PERFORMANCE(<?php echo strtoupper($form.'  '.$en.'    -   '.$term.'  '.$year); ?>)"
	},
      axisX:{
