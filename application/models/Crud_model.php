...
class Crud_model extends CI_Model {
    ...
    function get_grade($mark_obtained,$school_id=0) {
        
		if($school_id >0) $this->db->where('school_id' , $school_id);
...
//        $this->email_model->notify_email('new_message_notification', $this->db->insert_id());
    ...
    function delete_student($student_id) {
      ...
      $tables = array('student', 'attendance', 'enroll', 'invoice', 'mark', 'payment');
    ...
    function create_group($type='principal'){
      $data = array();
	  ...
	  $id = $type.'_id';
      if(!empty($_POST['user'])) {
          array_push($_POST['user'], $this->session->userdata('login_type').'_'.$this->session->userdata($id));
      ...
      redirect(site_url($type.'/group_message'), 'refresh');
    }
    // Group messaging portion
    function update_group($thread_code = "",$type='principal'){
      $data = array();
      $data['group_name'] = $this->input->post('group_name');
	  $id = $type.'_id';
      if(!empty($_POST['user'])) {
	...
	function sendnotifications($id,$role_id, $data, $message)
	{
		...
		if(($i%500==0 || $i==(count($notifi_userlist)-1)))
		{
			$url = 'https://fcm.googleapis.com/fcm/send';
					...
					$headers[] = 'Authorization: key= AAAAlwG5y_I:APA91bEjYdURNMbJWIeIovbk7Z8VhO-rRer7ifK1a3m9bFY8N-fm6AnmY0aXI0IX-3piHrxmwAaUIRhuikLduh-DULoxzATTMP_H15pzq2hU_xttuXldR2UgF0bR_jiUwPapqTWUS5XA'; // key here
