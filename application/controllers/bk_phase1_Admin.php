...
class Admin extends CI_Controller
{
  ...
  function student_profile($student_id)
  {
	...
	function student_report($class_id = '', $report='')
	{
		if ($this->session->userdata('principal_login') != 1)
            redirect(site_url('login'), 'refresh');
		...
		$page_data['page_title'] = get_phrase($report_title). " - ".get_phrase('class')." : ".
		$this->crud_model->get_class_name($class_id);
		$page_data['class_id'] 	= $class_id;
...
if($this->input->post('emarks') != null){
    ...
    function subject($param1 = '', $param2 = '' , $param3 = '')
    {
        ...
        if ($param1 == 'create') {
			...
			$cnt = $this->db->get_where('subject' , array('class_subject' => $class_subject,'class_id' => $class_id,'section_id' => $section_id))->num_rows();		
	...
	function behaviours($param1 = '', $param2 = '')
    {
        ...
        $page_data['behaviours'] = $this->db->get_where('behaviours' , array('school_id' => $school_id))->result_array();
	...
	function behaviour_content($behaviour_id = '')
    {
		...
		$school_id = $this->session->userdata('school_id');
        if ($behaviour_id == '')
            $behaviour_id  =   $this->db->get_where('behaviours' , array('school_id' => $school_id))->first_row()->id;
	...
	function class_subjects($param1 = '', $param2 = '')
    {
        ...
        $page_data['page_title'] = get_phrase('class_subjects');
        $this->load->view('backend/index', $page_data);
    }
    ...
    function section($class_id = '')
    {
        ...
        $page_data['page_title'] = get_phrase('manage_streams');
        $page_data['class_id']   = $class_id;
		 $page_data['school_id']   = $school_id;
        $this->load->view('backend/index', $page_data);
    }
    ...
    function generate_bulk_student_csv($class_id = '', $section_id = '')
    {
        ...
        $line   = array('Student Name', 'Admission Number', 'Parent Name', 'Parent Phone',  'Parent Email');
        fputcsv($file, $line, ',');
       ...
       if ($param1 == 'import') {
          if ($this->input->post('class_id') != '' && $this->input->post('section_id') != '') {
             ...
             foreach ($csv as $row) {
				  ...
				  $parent_id = $this->db->get_where('parent' , array('phone' => $row[3]))->row()->parent_id;
...
//                    $data2['roll']        = $row[1];
    ...
    function generate_bulk_teacher_csv()
    {
        ...
        $line   = array('Teacher Name', 'Role', 'Designation', 'Class', 'Section', 'Subject', 'Full / Part Time', 'Phone Number', 'Email');
        fputcsv($file, $line, ',');
