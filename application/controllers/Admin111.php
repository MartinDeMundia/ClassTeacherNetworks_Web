  ...
  function class_time_table_view(){
	...
	function insertimetablelesson($year,$term,$class_id,$section_id,$day,$startime,$endtime,$sectioname){
		       ...
		       $term = urldecode($term);
	   		   ...
	   		   $q="SELECT * FROM subject s JOIN class_subjects c ON c.id =  s.class_subject AND c.school_id = ".$schoolid." AND s.class_id= '".$class_id."' AND s.section_id = '".$section_id."' ORDER BY RAND() LIMIT 1";
			   $qsubject =$this->db->query($q);
			   $rowSubject =   $qsubject->row();

		       $subject = $rowSubject->id;
