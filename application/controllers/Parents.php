...
class Parents extends CI_Controller
{
    ...
    function __construct()
    {
		...
		//if ($this->session->userdata('parent_login') != 1) $this->clearsess();		
    }
    ...
    function teacher_list($param1 = '', $param2 = '', $param3 = '')
    {
		...
		$enroll = $this->db->get_where('enroll', array('student_id' => $param1))->row();
        ...
        $page_data['page_title'] = get_phrase('class_subject_teachers');
        $this->load->view('backend/index', $page_data);
    }
    ...
    function marks($param1 = '')
    {
        ...
        $page_data['page_title'] = get_phrase('education_progress');
        $this->load->view('backend/index', $page_data);
    }
    ...
    function class_routine_print_view($class_id , $section_id, $school_id,$student_id)
    {
        if ($this->session->userdata('parent_login') != 1)
            redirect('login', 'refresh');
        $page_data['class_id']   =   $class_id;
        ...
        $page_data['student_id'] =   $student_id;
        $this->load->view('backend/parent/class_routine_print_view_new' , $page_data);
    }
    ...
    function noticeboard($param1 = '', $param2 = '', $param3 = '')
    {
		...
		$school_id = $this->db->get_where('student',array('student_id'=>$param1))->row()->school_id;
	...
	function media($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        $page_data['medias'] = $this->db->get_where('media',array('class_id'=>$class_id,'section_id'=>$section_id,'status'=>1))->result_array();
	...
	function events($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        $page_data['events'] = $this->db->get_where('events',array('school_id'=>$school_id))->result_array();
    ...
    function group_message($param1 = "group_message_home", $param2 = ""){
	  ...
	  if ($param1 == "create_group") {
        $this->crud_model->create_group('parents');
      }
      elseif ($param1 == "edit_group") {
        $this->crud_model->update_group($param2,'parents');
      }
	...
	function manage_notification($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'update_notification') {
            ...
            redirect(site_url('parents/manage_notification/'), 'refresh');
        }
	...
	function feedback($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'update_feedback') {
			...
			$this->db->where('parent_id', $this->session->userdata('parent_id'));
			$this->db->update('parent', array('feedback' => $data['feedback']
			));
			$this->session->set_flashdata('flash_message', get_phrase('updated'));
             
            redirect(site_url('parents/feedback/'), 'refresh');
        }
  ...
  function class_layout($class_id ='' , $section_id = '')
    {
        if ($this->session->userdata('parent_login') != 1)
            redirect(site_url('login'), 'refresh');
           
        $page_data['page_name']  = 'class_layout';
        $page_data['class_ids']  =   $class_id;
        $page_data['section_ids']  =   $section_id;
