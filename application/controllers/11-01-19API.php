	...
	function getAccessTeachersDetails()
    {
		...
		if($cnt > 0 ){						
	       ...
	       $array_tt_settings = $query_timetable_settings->result_array();
    	    ...
    	    $subjects_get = $this->db->query("SELECT * FROM `class_routine_time_table` WHERE `period` = $d AND `day` in ('monday','tuesday','wednesday','thursday','friday') AND `year` LIKE '$running_year' and subject_id in ($subject_id)")->result_array();			
