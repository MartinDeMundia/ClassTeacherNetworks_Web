...
class API extends CI_Controller
{
	...
	function getMyClass($user_id,$role_id,$sel_section_id,$sel_class_id){  
		...
		if(count($sections) > 0){			 
			...
			$this->db->select('GROUP_CONCAT(DISTINCT subject.class_subject SEPARATOR ",") as subject_id,GROUP_CONCAT(DISTINCT subject.name SEPARATOR ",") as subject_name');
...
//echo json_encode($myclasses); exit();
		return $myclasses;
	}
...
///var_dump("SELECT  subject.*, class_subjects.* FROM  subject LEFT JOIN  class_subjects ON class_subjects.id = subject.class_subject WHERE   class_subjects.school_id =".$schoolid." AND class_subjects.is_elective <> 2 GROUP BY subject.class_subject"); exit();
        ...
        */
		...
		/*	if( count($subject_row) > 0){
			...
			$myclass = $this->getMyClass($user_id,$role_id,$section_id,$class_id);	
			//var_dump($myclass); exit();
...
/*	echo '<pre>';
    ...
    function getForumComments($forum_id)
	{
		...
		if(count($comments_data) > 0){
			...
			foreach ($comments_data as $row) {
				...
				$comment['ismuted'] = $this->db->get_where('forums_mstatus', array('userid' => $row['sender_id']))->row()->mstatus;
	...
	function getForumsListComments($forum_id=0,$iscomments=0,$user_id=0,$role_id=0,$student_id=0,$type='',$status='',$last_id='')
	{
		...
		if(count($forums_data) > 0){
			...
			foreach ($forums_data as $row) {
				...
				$forum['designation'] = $this->db->get_where('teacher', array('teacher_id' => $row['sender_id']))->row()->designation;
				$forum['ismuted'] = $this->db->get_where('forums_mstatus', array('userid' =>$sender_id))->row()->mstatus;
	...
	function getAccessStudentHealth()
	{
		...
		///$healths['report_download_link']= $this->report($student_id,1); temporary
	...
	function getUploadAssignments(){ 	          
		...
		if($user_id > 0 ){		
			...
			$this->db->insert('assignments_solutions', $data);	 	
			$media_id = $this->db->insert_id();	
    ...
    {		 
		...
		if($user_id > 0 ){							
			...
			if(count($media_data) > 0){
               ...
               //create the assignments notifications here
			   if($role_id == 1){
						...
						$assignment_data = $this->db->get_where('assignments', array('class_id' => $class_id,'section_id' => $section_id))->result_array();

					    if(count($assignment_data) > 0){
					
								foreach ($assignment_data as $row_assignments) {

											$fileArray = explode("/", $row_assignments['filepath']);
											$file_parts = pathinfo(end($fileArray));
											$file_parts['extension'];									
											$cool_extensions = Array('pdf','doc','xls','xlsx','jpg','gif','png','txt');

											if (in_array($file_parts['extension'], $cool_extensions)){
											   $myfile = $row_assignments['filepath'];	

												$media_assignments['id'] = $row_assignments['assignment_id'];
												$media_assignments['title'] = "ASSIGNMENT : ".$row_assignments['title'];
												$media_assignments['details'] = $row_assignments['description'];
												$media_assignments['file_name'] = $row_assignments['filepath'];
												$media_assignments['url'] = $row_assignments['filepath'];
												$media_assignments['date_time'] = $row_assignments['given_date'];
												$media_assignments['uploader_id'] = $row_assignments['user_id'];
												$media_assignments['uploader_name'] = ($row_assignments['role_id']== 2)?$this->db->get_where('teacher', array('teacher_id' => $row_assignments['user_id']))->row()->name:$this->db->get_where('principal', array('principal_id' => $row_assignments['user_id']))->row()->name;
												$media_assignments['class_id'] = $row_assignments['class_id'];
												$media_assignments['class_name'] = $this->db->get_where('class', array('class_id' => $row_assignments['class_id']))->row()->name;
												$media_assignments['section_id'] = $row_assignments['section_id'];
												$media_assignments['section_name'] = $this->db->get_where('section', array('section_id' => $row_assignments['section_id']))->row()->name;
												$media_assignments['visible'] = $row_assignments['status'];

                                    array_push($medias,$media_assignments);
                                  }
    ...
    {		 
		...
		if($user_id > 0 ){			 				
			...
			if($role_id == 1){
				...
				$qryData = "
                  SELECT * FROM  notifications WHERE  student_id = (SELECT student_id FROM parent WHERE parent_id =  '".$user_id."') ORDER by created_on DESC LIMIT 0,5
                ";                                                       
                $notification_data = $this->db->query($qryData)->result_array();
				
				
			}
			else{				
				$qryData = "
                  SELECT * FROM  notifications WHERE  receiver_id = '". $user_id."' AND receiver_role = '".$role_id."' ORDER by created_on DESC LIMIT 0,5
                ";                                                       
                $notification_data = $this->db->query($qryData)->result_array();
	...
	function getSaveClasses(){
		...
		$school_details = $this->db->query("SELECT * FROM school WHERE LCASE(school_name) = '".strtolower($schoolname)."'");
		$dbrowSchool =  $school_details->result(); 
		if(count($dbrowSchool) > 0){
			$school_id = $dbrowSchool[0]->school_id; 
		}else{
			$school_id = 0;
		}

		$teachers_details = $this->db->query("SELECT teacher_id FROM teacher WHERE phone = '".$phonenumber."'");
		$dbrows =  $teachers_details->result();                      
		$teacher_id = $dbrows[0]->teacher_id;

       //insert into class
		$class_details = $this->db->query("SELECT * FROM class WHERE LCASE(name) = '".strtolower($classname)."' AND school_id='".$school_id."'");
		$dbrowClass =  $class_details->result(); 
		if(count($dbrowClass) > 0){
			$class_id = $dbrowClass[0]->class_id; 
		}else{
	   ...
	   //insert into stream
		$stream_details = $this->db->query("SELECT * FROM section WHERE LCASE(name) = '".strtolower($stream)."' AND school_id='".$school_id."' AND class_id='".$class_id."'");
		$dbrowStream =  $stream_details->result(); 
		if(count($dbrowStream) > 0){
			$stream_id = $dbrowStream[0]->section_id; 
			//update
			$data = array(
				'teacher_id'=>$teacher_id				
			   );
			   $this->db->where('section_id',$stream_id);
			   $this->db->update('section',$data);
		}else{
			//insert new stream here
			$data = array(
			'name'=>strtoupper($stream),
			'nick_name'=>$stream,
			'school_id'=>$school_id,
			'teacher_id'=>$teacher_id,
			'class_id'=>$class_id,
		...
		$sql = "
		   ...
		   FROM
			   teacher t
			   LEFT JOIN
			   class c ON c.teacher_id = t.teacher_id
				   AND c.school_id = t.school_id
				   LEFT JOIN
			   section s ON s.teacher_id = t.teacher_id
				   AND s.class_id = c.class_id
				   AND s.school_id = t.school_id
				   LEFT JOIN
			   subject subj ON subj.teacher_id = t.teacher_id
				   AND subj.section_id = s.section_id
				   AND subj.class_id = c.class_id
			   LEFT JOIN school sch ON sch.school_id = t.school_id
		   WHERE
			   t.teacher_id 
			   = '".$teacher_id."' GROUP BY s.section_id , c.class_id ORDER BY s.name , c.name";

	   $teachers_info = $this->db->query($sql);
	   $response = array();
	   $teachersClassesArray = array();
	   foreach($teachers_info->result() as $classes_streams) {
		   $teachersClassesArray["name"] = $classes_streams->name;
		   $teachersClassesArray["middle_name"] = $classes_streams->middle_name; 
		   $teachersClassesArray["last_name"] = $classes_streams->last_name; 
		   $teachersClassesArray["tsc_number"] = $classes_streams->tsc_number; 
		   $teachersClassesArray["sex"] = $classes_streams->sex;
		   $teachersClassesArray["phone_verified"] =$classes_streams->phone_verified;
		   $teachersClassesArray["email"] =$classes_streams->email; 
		   $teachersClassesArray["password"] =$classes_streams->password; 
		   $teachersClassesArray["school_id"] =$classes_streams->school_id; 
		   $teachersClassesArray["designation"] =$classes_streams->designation; 
		   $teachersClassesArray["details"] =$classes_streams->details; 
		   $teachersClassesArray["teacher_id"] =$classes_streams->teacher_id;	
		   $teachersClassesArray["teacherscode"] =$classes_streams->teacherscode;			
		   $teachersClassesArray["initial"] =$classes_streams->initial; 
		   $teachersClassesArray["stream_id"] =$classes_streams->stream_id; 
		   $teachersClassesArray["stream_name"] =$classes_streams->stream_name; 
		   $teachersClassesArray["class_id"] =$classes_streams->class_id; 
		   $teachersClassesArray["class_name"] =$classes_streams->class_name; 
		   $teachersClassesArray["subject_id"] =$classes_streams->subject_id; 
		   $teachersClassesArray["subject"] =$classes_streams->subject; 
		   $teachersClassesArray["school_name"] =$classes_streams->school_name;               
		   array_push($response, $teachersClassesArray);
	   }

	   $classes_stream = $teachers_info->result()[0] ;

	   //get the schools in the system
	   $sql = "SELECT * FROM school;";
	   $response_schools = array();
	   $schools_records_set = $this->db->query($sql);
	   foreach($schools_records_set->result() as $schools) {
		   $rs_schools["school_id"] = $schools->school_id;
		   $rs_schools["school_name"] = $schools->school_name;
		   $rs_schools["school_logo"] = $schools->logo;
		   $rs_schools["website"] = $schools->website;
		   $rs_schools["email"] = $schools->email;
		   $rs_schools["address"] = $schools->address;
		   $rs_schools["telephone"] = $schools->telephone;
		   array_push($response_schools, $rs_schools);
	   }
	   $status = 1;
	   $message = "Successfully fetched Teachers details";

	   $response = array('status'=>$status,
	   'message'=>$message,
	   'teacher_id'=>$classes_stream->teacher_id,
	   'name'=>$classes_stream->name,
	   'middle_name'=>$classes_stream->middle_name,
	   'last_name'=>$classes_stream->last_name,
	   'tsc_number'=>$classes_stream->tsc_number,
	   'sex'=>$classes_stream->sex,
	   'phone_verified'=>$classes_stream->phone_verified,
	   'email'=>$classes_stream->email,
	   'password'=>$classes_stream->password,
	   'school_id'=>$classes_stream->school_id,
	   'designation'=>$classes_stream->designation,
	   'details'=>$classes_stream->details,
	   'teacherscode'=>$classes_stream->teacherscode,
	   'initial'=>$classes_stream->initial,
	   'stream_id'=>$classes_stream->stream_id,
	   'stream_name'=>$classes_stream->stream_name,
	   'class_id'=>$classes_stream->class_id,
	   'class_name'=>$classes_stream->class_name,
	   'subject_id'=>$classes_stream->subject_id,
	   'subject'=>$classes_stream->subject,
	   'school_name'=>$classes_stream->school_name,		
	   'classlist'=>$response,
	   'schoolslist'=>$response_schools);
	   echo json_encode($response);  

	}

	function getSaveStudentDetails(){
		$status = 0;
		$parentsphone  = $this->input->post('parentsPhone');
		$schoolname  = $this->input->post('schoolname');
		$studnemis  = $this->input->post('studnemis');
		$studfirstname  = $this->input->post('studfirstname');
		$studsecondname  = $this->input->post('studsecondname');
		$studlastname  = $this->input->post('studlastname');
		$studgender  = $this->input->post('studgender');
		$studbirthday  = $this->input->post('studbirthday');
		$studentcode  = $this->input->post('studentcode');


       /*
		$parentsphone  = $this->input->get('parentsPhone');
		$schoolname  = $this->input->get('schoolname');
		$studnemis  = $this->input->get('studnemis');
		$studfirstname  = $this->input->get('studfirstname');
		$studsecondname  = $this->input->get('studsecondname');
		$studlastname  = $this->input->get('studlastname');
		$studgender  = $this->input->get('studgender');
		$studbirthday  = $this->input->get('studbirthday');
		$studentcode  = $this->input->get('studentcode');*/

		$parents_details = $this->db->query("SELECT parent_id FROM parent WHERE phone LIKE '".$parentsphone."'");
		$dbrows =  $parents_details->result(); 
        $parentid = $dbrows[0]->parent_id;


		$school_details = $this->db->query("SELECT * FROM school WHERE school_name LIKE '".$schoolname."'");
		$dbrowSchool =  $school_details->result(); 
		if(count($dbrowSchool) > 0){
			$school_id = $dbrowSchool[0]->school_id; 
		}else{
		...
		//check student
		$stud_details = $this->db->query("SELECT nemis,student_id FROM student WHERE nemis LIKE '".$studnemis."'");
		$dbrowStud =  $stud_details->result(); 
		if(count($dbrowStud) > 0){
		   $this->db->query("DELETE FROM student WHERE nemis = '".$studnemis."'");
		   $this->db->query("DELETE FROM enroll WHERE student_id = '".$dbrowStud[0]->student_id."'");
		}


		$data['student_code'] = $studentcode;
		$data['name'] = $studfirstname;
		$data['middle_name'] = $studsecondname;
		$data['last_name'] = $studlastname;
		$data['sex'] = $studgender;
		$data['birthday'] = $studbirthday;
		$data['nemis'] = $studnemis;
		...
		$message = "Successfully added student ".$studfirstname;		
		//get siblings
				$sql = "
			   ...
			   $response_parent_student_list = array();
			   foreach($parent_to_student->result() as $studlist) {
				   ...
				   array_push($response_parent_student_list, $aListnotessiblings);
			   }
			   ...
			   $sql = "
			   SELECT parent_id,name,middle_name,email,phone,phone2,address,profession FROM parent WHERE parent_id
				= '".$parentid."'";
				  ...
				  $dbrows =  $parent->result();
				   ...
				   $names = explode(" ",$dbrows[0]->name);
					...
					$response = array('status'=>$status,
						'message'=>$message,
						'parentid'=>$dbrows[0]->parent_id,
						'firstname'=>$dbrows[0]->name,
						'lastname'=>$dbrows[0]->middle_name,
						'phone1'=>$dbrows[0]->phone,
						'phone2'=>$dbrows[0]->phone2,
						'email'=>$dbrows[0]->email,
						'occupation'=>$dbrows[0]->profession,
						'parentstudentlist'=>$response_parent_student_list,
						'schoolslist'=>$response_schools,
						'anote'=>"");
			   }else{
...
/*
	...
	function getSchoolInfo(){
		...
		//$school = "FAIRMOUNT  JUNIOR SCHOOL";


		$dbrows = array();
		$sql = " SELECT * FROM school WHERE school_name = '".$school."'";
		$schooldata = $this->db->query($sql);
		$countsRecords =  $schooldata->num_rows();
		if( $countsRecords > 0){
		  $dbrows =  $schooldata->result()[0];
		  $school_id = $dbrows->school_id;
		}else{
		  $school_id = 0;
		} 

		//get the sections (sub classes) in the system
		$sql = "SELECT * FROM section WHERE school_id = '".$school_id."';";
		$response_sections = $r_sections_data =  array();
		$sections_records_set = $this->db->query($sql);
		foreach($sections_records_set->result() as $sections) {
			$response_sections["class_id"] = $sections->section_id;
			$response_sections["class_name"] = $sections->name;
			$response_sections["teacher_id"] = $sections->teacher_id;			
			array_push($r_sections_data, $response_sections);
		}


		//get the sections (sub classes) in the system
		$sql = "SELECT * FROM class WHERE school_id = '".$school_id."';";
		$response_class = $r_class_data =  array();
		$class_records_set = $this->db->query($sql);
		foreach($class_records_set->result() as $classes) {
			$response_class["class_id"] = $classes->class_id;
			$response_class["class_name"] = $classes->name;
			$response_class["teacher_id"] = $classes->teacher_id;			
			array_push($r_class_data, $response_class);
		}		
		
		$response = array(		
		'streamlist'=>$r_sections_data,
		'classlist'=>$r_class_data,
		);
	...
	function getSaveSchoolDetails(){
		$status = 0;
		$school  = $this->input->post('school');
		$schooladdress  = $this->input->post('schooladdress');
		$schoolcounty  = $this->input->post('schoolcounty');
		$schooltelephone  = $this->input->post('schooltelephone');
		...
		$data['county'] = $schoolcounty;
		$data['telephone'] = $schooltelephone;
		$data['email'] = $schoolemail;
		$data['website'] = $schoolwebsite;
	...
	function getSaveTeacherDetails(){
		$status = 0;
		  ...
		  $tscnumber  = $this->input->post('tscnumber');
		  $gender  = $this->input->post('gender');
		  $emailaddress  = $this->input->post('emailaddress');
		  $passwd  = $this->input->post('passwd');
		  $teacherscode  = $this->input->post('teacherscode'); 
		  ...
		  if($accounttype == "teacher"){
					...
					$sql = "
					SELECT * FROM teacher WHERE phone
					= '".$phonenumber."'";
					$teacherdata = $this->db->query($sql);          
					$countRecords =  $teacherdata->num_rows();

					if( $countRecords > 0){
						$dbrows =  $teacherdata->result();
						//do update          
						$data = array(
						'initial'=>$initial,
						'name'=>$firstname,
						...
						$message = "Teachers details saved!";
					}else{
						...
						$message = "Teacher details added"; 
								...
								//check if the teacher exist 
								$sql = "
								SELECT * FROM principal WHERE phone
								= '".$phonenumber."'";
								$principaldata = $this->db->query($sql);          
								$countRecords =  $principaldata->num_rows();
			
								if( $countRecords > 0){
									$dbrows =  $principaldata->result();
									//do update          
									$data = array(
									'initial'=>$initial,
									'name'=>$firstname,								
									...
									$message = "Principal details saved!";
								}else{
									...
									$message = "Principal details added"; 
								}	
    ...
    function getSaveParentDetails(){
      ...
      $parentid  = $this->input->post('parentid');
		...
		//check if the parent exist // disabled this on new release
	    $sql = "
        ...
        if( $countRecords > 0){
          ...
          //create a new parent and map student to parent         
           ...
           $otp = rand(1000, 9999);
			...
			$data['password'] = sha1($passwd);
			$data['sound'] = 1;
			$data['middle_name'] = $lastname;			
			$data['otp'] = $otp;
	...
	function removeSubject(){
		...
		";
        $subject_info = $this->db->query($sql);
		$countRecords =  $subject_info->num_rows();
		if($countRecords > 0 ){
			$dbrows =  $subject_info->result();
            $class_subject_id = $dbrows[0]->class_subject;

			$sql = "DELETE FROM subject WHERE subject_id ='".$subject_id."' ";
			$this->db->query($sql);

			$sql = "DELETE FROM class_subjects WHERE id ='".$class_subject_id."' ";
			$this->db->query($sql);
	...
	function deleteStudent(){
		...
		$sql = "DELETE FROM enroll WHERE student_id ='".$student_id."'";
		$this->db->query($sql);	
		$message = "Successfully deleted!";
	...
	function deleteClassStream(){
		$stream_id  = $this->input->post('stream_id');
		$class_id  = $this->input->post('class_id');
		$teachers_id  = $this->input->post('teachers_id');
		$status = 0;

		$sql = "DELETE FROM section WHERE class_id ='".$class_id."' AND section_id='".$stream_id."' ";
		$this->db->query($sql);
    ...
    function getTeachersInfo(){
         ...
         $phonenumber  = $this->input->post('phonenumber');
		 //$phonenumber  = "0722420978";
		 //$phonenumber  = "0720000000";
		 //$phonenumber  = "0725000000";
        ...
        $teachers_info_classlist = $this->db->query($sql);
        ...
        $teachers_info = $this->db->query($sql);
		$countRecords =  $teachers_info->num_rows();

		if($countRecords <= 0 ){
			//fetch principal
					$sql = "
					SELECT 
						p.principal_id teacher_id,
						p.name,
						p.middle_name,
						p.last_name,
						p.sex,
						...
						p.initial,
						sch.school_name,
						'principal' teacherscode
					FROM
						principal p
							LEFT JOIN
						school sch ON sch.school_id = p.school_id
					WHERE
						p.phone = '".$phonenumber."'
					";
        ...
        $response = array();
        $teachersClassesArray = array();
        foreach($teachers_info_classlist->result() as $classes_streams) {
			$teachersClassesArray["teacher_id"] = $teacher_id;
            $teachersClassesArray["name"] = $classes_streams->name;
            $teachersClassesArray["middle_name"] = $classes_streams->middle_name; 
            $teachersClassesArray["last_name"] = $classes_streams->last_name; 
            $teachersClassesArray["tsc_number"] = $classes_streams->tsc_number; 
...
$response = array('status'=>$status,
    ...
    /*
       ...
       $sql = "
        SELECT parent_id,name,middle_name,email,phone,phone2,address,profession,password FROM parent WHERE parent_id
         = '".$parentid."'";
        $parent = $this->db->query($sql);          
        $countRecords =  $parent->num_rows();
        if( $countRecords > 0){
    ...
    {		 
		...
		$response = array('status'=>'0','message'=>'student id missing');
		//echo json_encode("user_id= ".$user_id  ." role_id= ".$role_id ." student_id= ".$student_id ." topic= ".$topic ." from= ".$from ." to= ".$to ." type= ".$type ." class_id= ".$class_id ." section_id= ".$section_id  ); exit();
		...
		if($type > 0 ){	
			...
			foreach($userdata as $v){
				...
				if($role == 'parent'){					
					...
					$noti_arr['title'] = 'New Forum Message!';
					$noti_arr['content'] = $topic;				
					$noti_arr['type'] = '12';	
					...
					if($role == 'principal') 			
						...
						$tdata = $this->db->get_where('teacher', array('school_id' => $school_id))->result_array();
						
						foreach($tdata as $trows){
						    $noti_arr['receiver_id'] = $trows['teacher_id'];
						    //////$this->db->insert('notifications', $noti_arr);
						}				
...
/*				$report_name = 'fee_report';
			...
			case 6:
                ...
                $sql = "
	                ...
	                if($stream_id >0){
					   ...
					   //$logo =base_url('/uploads/logo.png'); //temporary					 
	...
	function fetchtimetablelessons(){		
		$qryTData = "
			SELECT t.day,t.tslots,t.venue,s.name subject,te.name teacher,te.teacher_id FROM timetable t 
			JOIN subject s ON s.subject_id = t.subject
			JOIN teacher te ON te.teacher_id = t.teacher
			WHERE  (day) =   DAYNAME(CURDATE())  AND t.year = '".date('Y')."' AND t.school = 34 AND t.term = 'Term 1' GROUP BY t.teacher;
		";                                                       
		$timetable_data = $this->db->query($qryTData)->result_array();		
		foreach ($timetable_data as $row) {			
			$datatimetable['title'] = "Lesson for Tr.".$row['teacher'];
            $datatimetable['content'] = "Class [".$row['venue']."] ,".$row['subject']." at :".$row['tslots'];
            $datatimetable['type'] = 0;
            $datatimetable['type_id'] = 1988;
            $datatimetable['student_id'] = 0;
            $datatimetable['receiver_id'] = $row['teacher_id'];
            $datatimetable['receiver_role'] = 2;
            $datatimetable['is_read'] = 0;
            $datatimetable['creator_id'] = 0;
            $datatimetable['creator_role'] = 2;
			$datatimetable['created_on'] = date('Y-m-d H:i:s');   
            $this->db->insert('notifications', $datatimetable);  			
		}		
	}
	...
	function fcmpush(){		
		...
		foreach ($notification_data as $row) {		
			if($row['receiver_role'] == 1){
				$student_id = $row['student_id'];
                $receiver_id = $this->db->get_where('student', array('student_id' => $student_id))->row()->parent_id;				
			...
			//var_dump($arrNotification); echo "<hr>";
			$regId = "/topics/all";
	...
	function getCreateJournal(){
		...
		$data['student_id']  = $student_id;		
        $this->db->insert('student_journal', $data);
        $jid = $this->db->insert_id();		
        //$response = array('status'=>'0','message'=>'created a journal !','id'=>$jid);
        //$this->getJournalList();		
		$sql = "SELECT *  FROM student_journal WHERE student_id = '".$student_id."' ORDER BY journal_date DESC";
        $journal_records = $this->db->query($sql);
        $response = array();
        foreach($journal_records->result() as $journals) {
            $ajournals["id"] = $journals->id;
			$ajournals["user_id"] = $journals->user_id;
            $ajournals["role_id"] = $journals->role_id;
			$ajournals["emoji"] = $journals->emoji;
            $ajournals["explanation"] = $journals->explanation;	
            $ajournals["journal_date"] = date("F jS, Y", strtotime($journals->journal_date)) ;
            $ajournals["student_id"] = $journals->student_id;        
            array_push($response, $ajournals);
        }
        $response = array('status'=>'1','message'=>'success','journals'=>$response);
        echo json_encode($response);		
    }	
	 ...
	 function getJournalList(){
        ...
        //$sql = "SELECT *  FROM student_journal a LEFT JOIN student p ON p.parent_id = a.student_id  WHERE student_id = '".$user_id."'";
		$sql = "SELECT *  FROM student_journal WHERE student_id = '".$student_id."' AND DATE(journal_date) = '".date('Y-m-d')."' ORDER BY journal_date DESC";
        $journal_records = $this->db->query($sql);
        $response = array();
        foreach($journal_records->result() as $journals) {
            $ajournals["id"] = $journals->id;
   ...
   function getJournalListDate(){
        $date  = $this->input->post('jDate');
		$pdate = urldecode($date);		
		$newd = date("Y-m-d H:i:s", strtotime($pdate));		
		//$new_time = strtotime($newd . "+3hours");
		$mdate = date("Y-m-d",$new_time);
        $student_id  = $this->input->post('StudentId'); 		

		$sql = "SELECT *  FROM student_journal WHERE student_id = '".$student_id."' AND DATE(journal_date) LIKE '".$pdate."' ORDER BY journal_date DESC";
  //echo json_encode($sql);
	...
	function saveTeachersSubjects(){
		...
		$sqlTeacher = "SELECT * FROM teacher WHERE teacher_id = '".$teachers_id."' ";
		$queryDataTeacher = $this->db->query($sqlTeacher)->result_array();
		if(count($queryDataTeacher)){ 
			$school_id = $queryDataTeacher[0]["school_id"]; 
		} 
		$sqlSel = "
		SELECT * FROM class_subjects WHERE subject = '".$subject."' AND school_id = '".$school_id."' 
		";
		$queryDataSubj = $this->db->query($sqlSel)->result_array();
		if(count($queryDataSubj)){
			$class_subject_id = $queryDataSubj[0]["id"];
		}else{
			$data = array(
			   ...
			   $class_subject_id = $this->db->insert_id();
			   if( $this->db->trans_status() === FALSE ){
				 $this->db->trans_rollback();
			   }
			   else{
		...
		//insert into subject
		$sqlSel = "
		SELECT * FROM subject WHERE name = '".$subject."' AND class_id = '".$class_id."' AND section_id = '".$stream_id."' AND teacher_id = '".$teachers_id."'
		";
		$queryDataSubj = $this->db->query($sqlSel)->result_array();
		if(count($queryDataSubj)){
			$data = array(
			...
			$this->db->where('subject_id',$queryDataSubj[0]["subject_id"]);
		...
		$sql = "
			...
			FROM
				subject
					LEFT JOIN
				class_subjects ON class_subjects.id = subject.class_subject
			WHERE
				subject.section_id = '".$stream_id."'
					AND subject.class_id = '".$class_id ."'
					AND subject.teacher_id = '".$teachers_id."'
					AND class_subjects.is_elective <> 2
			GROUP BY subject.class_subject
		";

		
	   $subject_records = $this->db->query($sql);
        $response = array();
        foreach($subject_records->result() as $teachers_subjects) {
            $aSubj["id"] = $teachers_subjects->subject_id;
			$aSubj["code"] = $teachers_subjects->subject_code;
            $aSubj["abbreviation"] = $teachers_subjects->subject_code;
			$aSubj["description"] = $teachers_subjects->subject;
            $aSubj["is_active"] = 1;	
            $aSubj["teacher_id"] = $teachers_subjects->teacher_id;
            $aSubj["class_id"] = $teachers_subjects->class_id;
			$aSubj["section_id"] = $teachers_subjects->section_id;
			$aSubj["school_id"] = $teachers_subjects->school_id;       
            array_push($response, $aSubj);
        }       
		$response = array('status'=>'1','message'=>'success','subjectlist'=>$response);
        echo json_encode($response);
    }
	...
	function saveStudentClasses(){
        $student_id  = $this->input->post('student_id');
		$stream_name  = $this->input->post('stream_id');
		...
		$sqlStudent = "
		SELECT 
			s.student_id, p.parent_id, p.phone,s.school_id
		FROM
			student s
				JOIN
		...
		";
		$queryDataStudent = $this->db->query($sqlStudent)->result_array();
		if(count($queryDataStudent)){
			$school_id = $queryDataStudent[0]["school_id"];
			$parentphone = $queryDataStudent[0]["phone"];
		}	

		$sqlClass = "
		SELECT * FROM class WHERE name = '".$class_name."' AND school_id = '".$school_id."' 
		";
		$queryDataClass = $this->db->query($sqlClass)->result_array();

		if(count($queryDataClass)){
			$class_id = $queryDataClass[0]["class_id"];
		}else{
			$data = array(
				'name'=>$class_name,
				'teacher_id'=>0,
		...
		$sqlStream = "
		SELECT * FROM section WHERE name = '".$stream_name."' AND school_id = '".$school_id."' 
		";
		$queryDataStream = $this->db->query($sqlStream)->result_array();

		if(count($queryDataStream)){
			$stream_id = $queryDataStream[0]["section_id"];
		}else{
			$data = array(
				'name'=>$stream_name,
				'teacher_id'=>0,
		...
		FROM
			student s
				LEFT JOIN
			enroll e ON e.student_id = s.student_id
				LEFT JOIN
			section sect ON sect.section_id = e.section_id
				LEFT JOIN
			class c ON c.class_id = e.class_id
				LEFT JOIN
	   ...
	   $student_records = $this->db->query($sql);
        $response = array();
        foreach($student_records->result() as $student_class) {
            $aClass["student_id"] = $student_class->student_id;
			$aClass["student_code"] = $student_class->student_code;
            $aClass["student_name"] = $student_class->student_name;
			$aClass["school_id"] = $student_class->school_id;         	
            $aClass["section_id"] = $student_class->section_id;
            $aClass["stream_name"] = $student_class->section_name;
			$aClass["class_id"] = $student_class->class_id;
			$aClass["class_name"] = $student_class->class_name;
			$aClass["teacher_id"] = $student_class->teacher_id; 
			$aClass["teacher"] = $student_class->teacher;  //teacherscode	
			$aClass["teacherscode"] = $parentphone;		
            array_push($response, $aClass);
			$school_id = $student_class->school_id; 
        }    
		...
		$response = array('status'=>'1','message'=>'success','studentclasslist'=>$response,'streamlist'=>$r_sections_data,
		'classlist'=>$r_class_data);
        echo json_encode($response);
    }
	...
	function getStudentClass(){
	   ...
	   $student_records = $this->db->query($sql);
        ...
        foreach($student_records->result() as $student_class) {
			...
			$aClass["class_name"] = $student_class->class_name;
			$aClass["teacher_id"] = $student_class->teacher_id; 
			$aClass["teacher"] = $student_class->teacher; 			
            array_push($response, $aClass);
			$school_id = $student_class->school_id; 
        }    
	...
	function checkPassword(){
		...
		$sql = "
			...
			FROM
				teacher
			WHERE phone ='".$phone."' AND password = '".sha1($passwd)."' 	
		";
	   $queryDataQTeacher = $this->db->query($sql)->result_array();		
	   if(count($queryDataQTeacher)){
			$id = $queryDataQTeacher[0]["teacher_id"];
		...
		WHERE phone ='".$phone."' AND password = '".sha1($passwd)."' 	
		";	
		$queryDataQPrincipal = $this->db->query($sql)->result_array();
		if(count($queryDataQPrincipal)){
			$id = $queryDataQPrincipal[0]["principal_id"];
			$is_password_correct = true;		
		}
		...
		WHERE phone ='".$phone."' AND password = '".sha1($passwd)."' 	
		";	
		$queryDataQParent = $this->db->query($sql)->result_array();
		if(count($queryDataQParent)){
			$id = $queryDataQParent[0]["parent_id"];
			$is_password_correct = true;		
		}
    
		$response = array('status'=>$is_password_correct,'message'=>'success');
        echo json_encode($response);
    }
	...
	function snoozeNotification(){
		...
		$data = array('id' => $notificationid);		
		$this->db->delete('notifications', $data);
		$response = array('status'=>'1','message'=>'Snoozed');
        echo json_encode($response);
	}

	function getMuteUser(){
	    $userid  = $this->input->post('user_id');
	    $mstatus  = $this->input->post('mstatus');
	    
	    $data = array('userid' => $userid);
	    $this->db->delete('forums_mstatus', $data);
	    
	    $data['userid']  = $userid;
	    $data['mstatus']  = $mstatus;	   
	    $data['created_on']  = date('Y-m-d h:i:s');	    
	    $this->db->insert('forums_mstatus', $data);	

	    
	    $response = array('status'=>'1','message'=>'Changed user forum status');
	    echo json_encode($response);
	}
