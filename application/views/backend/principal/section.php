...
<div class="row">
	<div class="col-md-12">
		...
		<div class="tabs-vertical-env">
			...
			<?php 
				$classes = $this->db->get_where('class' , array('school_id' => $row['school_id']))->result_array();
				foreach ($classes as $row):
			?>
				<li class="<?php if ($row['class_id'] == $class_id) echo 'active';?>">
					<a href="<?php echo site_url('admin/section/'.$row['class_id']);?>">
						<i class="entypo-dot"></i>
						<?php echo get_phrase('stream');?> <?php echo $row['name'];?>
					</a>
				</li>
			<?php endforeach;?>
			</ul>
					...
					<table class="table table-bordered responsive">
						<thead>
							<tr>
								...
								<th><?php echo get_phrase('abbreviations');?></th>
								<th><?php echo get_phrase('class_name');?></th>
								<th><div><?php echo get_phrase('total_seats');?></div></th>
								<th><div><?php echo get_phrase('couplings');?></div></th>
								<th><div><?php echo get_phrase('columns');?></div></th>
							...
							<tr>
								...
								<td>
									<?php 
										...
										elseif ($row['principal_id'] != '' && $row['principal_id'] != 0)
											echo 'Principal-'.$this->db->get_where('principal' , array('principal_id' => $row['principal_id']))->row()->name;
										?>
								</td>
								<td>
									<div class="btn-group">
