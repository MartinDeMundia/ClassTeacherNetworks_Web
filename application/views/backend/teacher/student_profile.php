...
<?php
  ...
  foreach ($student_info as $row):
    ...
    $class_id = $enroll_info->row()->class_id;
     $exams = $this->db->get_where('exam', array('school_id' => $this->session->userdata('school_id')))->result_array(); 
?>
<div class="profile-env">
	<header class="row">
        ...
        <span>
          ...
          <a href="<?php echo site_url('teacher/student_information/'.$enroll_info->row()->class_id);?>">
            <?php echo get_phrase('stream').' - '.$class_name.' | '. get_phrase('class').' - '.$section_name; ?>
          </a>
        </span>
      </center>
		</div>
    <div class="col-md-9">
      ...
      <!--li class="">
        ...
        <?php
          $basic_info_titles = ['name','parent', 'stream', 'class', 'email', 'phone', 'address', 'gender', 'birthday'];
           ...
           <tbody>
               <?php
                   ...
                   $subjects = $this->db->get_where('subject' , array(
                       'class_id' => $class_id , 'section_id' => $enroll_info->row()->section_id, 'year' => $running_year
                   ))->result_array();
