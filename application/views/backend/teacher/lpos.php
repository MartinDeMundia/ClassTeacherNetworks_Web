						...
						<div class="form-group" style="margin-bottom: 35px;">						
								<select name="sub" id="sub" class="form-control">
									...
									<?php 
										...
										$sql = "SELECT  subject  FROM  class_subjects WHERE  school_id = '".$this->session->userdata('school_id')."'   AND  is_elective <> 2";
                                        $subjects = $this->db->query($sql)->result_array();
										foreach($subjects as $row):
									?>
									<option value="<?php echo $row['subject'];?>"
										<?php if($subject_id == $row['subject']) echo 'selected';?>>
		...
		var exam=$("#examtype").val();
			...
			if (2==4){
				...
				$.ajax({
					...
					type: 'POST',
					url: '<?php echo base_url(); ?>index.php/teacher/s_report/create',
					data: p ,
					cache: false,
