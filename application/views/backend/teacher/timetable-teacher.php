...
<?php
    ...
    $teacher_id = $class_id;
    $ci =& get_instance();
    $school_id=$this->db->get_where("teacher", array('teacher_id'=>$teacher_id))->row()->school_id;
   ...
   $teachersname=$this->db->get_where("teacher", array('teacher_id'=>(int)$teacher_id))->row()->name;
...
<tr><td colspan="2" style="font-family:helvetica;font-weight:bold;">Tr. <?=$teachersname;?></td><td></td><td></td></tr>
<tr><td colspan="2" style="font-family:helvetica;font-weight:bold;"><?=$term;?> , <?=$year;?></td><td></td><td></td></tr>
...
<th style="background-color:<?=$headersbackground ; ?> color:<?=$headertcolor ; ?> font-size:<?=$headersize+0.2;?> ; width:90px;"><div>Day</div></th>
...
<?php
    ...
    function fetchtimetablelesson($year,$term,$teacher,$day,$starttime,$endtime){
      ...
      // $school_id = $ci->session->userdata('school_id'); //$ci->session->userdata('school_id')
       $school_id=$ci->db->get_where("teacher", array('teacher_id'=>$teacher))->row()->school_id;
       $bool = 0;
       ...
       $dbsettings =  $ci->db->get_where('timetable_settings', array(
        ...
        $displayString = "";
        if(count($queryData)){ 
          $displayString .= '<div style="height:10px;background-color:#E5E8F1 ; color:brown; width:100vh;font-size:1.4em;margin-top:20px;">'.$lessonbox.'</div>';
          foreach ($queryData as $key => $savedslots) { 
                   $venueboxcolor =  '#' . substr(md5($key.$savedslots), 0, 6);            
                   $displayString .= '<div stroke="0.3" fill="false" strokecolor="orange" color="orange" style="font-family:helvetica;font-weight:bold;font-size:3em;background-color:'.$venueboxcolor.';">'.$savedslots['venue'].'</div>';
        ...
        foreach ($days as $key => $day) {
          echo '<tr>';
            ...
            echo '<td style="width:90px;height:60px; background-color:'.$headersbackground.' color:'.$headertcolor.'">'.trim($day).'<div stroke="0.5" fill="true" strokecolor="grey" color="#94A3C2" style="font-family:helvetica;font-weight:bold;font-size:3em;">'.substr($day, 0, 1).'</div>

            </td>';

                   foreach ($timeslots as $key => $timeslot) {                      
                         $timeslotArray = fetchtimetablelesson($year,$term,$teacher_id,$day,$timeslot[0],$timeslot[1]);


                         if($timeslotArray['timeslotype'] != "") {
                                   print('<td  style="background-color:'.$bheadersbackground.' color:'.$bheadertcolor.' font-size:0.8em;font-weight: bolder;display:inline-block;text-align:center;">
