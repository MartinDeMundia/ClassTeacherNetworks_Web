    ...
    <div class="mail-sidebar" style="min-height: 800px;">
      ...
      <div class="mail-sidebar-row hidden-xs">
          <a href="#" onclick="showAjaxModal('<?php echo site_url('modal/popup/create_group/');?>');" class="btn btn-success btn-block">
              <?php echo get_phrase('create_forum_message_group'); ?>
          </a>
      </div>
        <!-- message user inbox list -->
        <ul class="mail-menu">
            <?php
			  $login_school_id = $this->session->userdata('school_id');
			  ...
			  $members = json_decode($row['members']);
				...
				if($login_school_id == $school_id) $flag = 1;  
                elseif (in_array($this->session->userdata('login_type').'_'.$this->session->userdata('login_user_id'), $members)) $flag = 1; 
              ...
              <li class="col-md-12 <?php if (isset($current_message_thread_code) && $current_message_thread_code == $row['group_message_thread_code']) echo 'active'; ?>">
                <div class="col-sm-10" style="text-align:left; margin: 0; padding: 0;">
                  <a href="<?php echo site_url('teacher/group_message/group_message_read/'.$row['group_message_thread_code']); ?>" style="padding:12px;">
