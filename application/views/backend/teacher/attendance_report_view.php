...
<div class="row">
	...
	$query = $this->db->get_where('class' , array('school_id' => $school_id));
    if ($query->num_rows() > 0):
...
<?php if ($class_id != '' && $section_id != '' && $month != '' && $sessional_year != ''): ?>
                        ...
                        <tr>
							...
							<?php
                                foreach ($attendance as $row1):
									...
									$subject_name = $this->db->get_where('subject', array('subject_id' => $row1['subject_id']))->row()->name;
									$period = $row1['period'];
									...
									if($status == 2)  {
										echo "$subject_name($period)<i class='entypo-record' style='color: #ee4749;'></i><br>"; 
									}
									else { 
										echo "$subject_name($period)<i class='entypo-record' style='color: #00a651;'></i><br>";
									}
    ...
    <?php ?>
            ...
            <center>
                <a href="<?php echo site_url('teacher/attendance_report_print_view/' . $class_id . '/' . $section_id . '/' . $month . '/' . $sessional_year); ?>"
