...
function showUser() {
    ...
    if (str == "") {
        ...
        xmlhttp.open("GET","<?php echo base_url();?>return.php?f="+str+"&st="+str2+"&s="+str3+"&y="+str4+"&t="+str5+"&exam="+str6,true);
        xmlhttp.send();
    }
}
                    ...
                    <div class="ibox-content">
							...
							<form method="POST" enctype="multipart/form-data" class="form-inline" role="form" id="ef">
							 ...
							 <div class="form-group  hidden" style="margin-bottom: 15px;">
								 ...
								 <div id="subject1" class="form-group hidden" style="margin-bottom: 15px;">
								  ...
								  $q="SELECT DISTINCT(Subject) AS Subject from subjectallocationa where schoo_id='".$this->session->userdata('login_user_id')."' ";
                            ...
                            <div class="col "><input placeholder="enter limit..." class="form-control" type="text" id="limit" disabled="true"/></div>
                          </div>
	 ...
	 <div class="col-lg-12">
                <div class="ibox">
					...
					<div class="form-group">
					 <a class=" " href="<?php echo base_url();?>uploads/data.xlsx" download="data sample file">Download blank file</a>
						<label for="field-1" class="col-sm-3 control-label"><?php echo get_phrase('select_excel_file');?></label>
                        
						
					</div>
					
					 <form action="<?php echo base_url();?>upload.php" class="dropzone" id="dropzoneForm">
                                <div class="fallback">
...
$(".i-checks").iCheck(function(){
	...
	$("#examst").change(function(){
		...
		$.ajax({
		type: 'POST',
		url:'<?php echo base_url();?>examfetch.php',
		data: id,
		dataType: 'json',
	...
	$("#entry").focusout(function() {
		...
		$.ajax({
				type:'POST',
				url:'<?php echo base_url();?>entry.php',
				data:dataString,
				cache:false,
    ...
    if (keycode == '13') {
		...
		if (no_error){
			...
			$.ajax({
				type:'POST',
				url:'<?php echo base_url();?>save.php',
				data:dataString,
				cache:false,
				success:function(result){
					//$("#paper").val('');
	...
	var id2=parseInt($("#examst").val());
		...
		if (no_error){
			...
			$.ajax({
				type:'POST',
				url:'<?php echo base_url();?>excel.php',
				data:dataString,
				cache:false,
				success:function(result){
					$("#gif").addClass('hidden');
